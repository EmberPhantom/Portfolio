...
export default function LiveStatus() {
  ...
  useEffect(() => {
    ...
    if (!wakatimeKey && !spotifyKey) {
      const statuses = [
        { type: 'coding', text: 'Building FORGE OS v2.0', detail: 'React & Tailwind' },
  ...
  return (
    <motion.div 
      ...
      className="inline-flex items-center gap-3 px-4 py-2 bg-forge-surface/80 border border-forge-muted/20 backdrop-blur-sm rounded-full mt-8"
    >
      <div className="relative flex items-center justify-center w-6 h-6 rounded-full bg-forge-black border border-forge-muted/30">
      ...
      <div className="flex flex-col text-left">
        <span className="text-xs text-gray-400 font-mono uppercase tracking-wider">{status.type === 'coding' ? 'Now Coding' : status.type === 'music' ? 'Now Playing' : 'Status'}</span>
        <span className="text-sm text-white font-medium leading-none">{status.text} <span className="text-gray-500 font-normal hidden sm:inline">— {status.detail}</span></span>
      </div>
    </motion.div>
  );
}
